...
export const RichTextClipboardExtension = Extension.create({
	...
	addProseMirrorPlugins() {
		...
		return [new Plugin({ props: { clipboardSerializer } })];
	},
});
...
function serializeLinkForClipboard(mark: Mark): DOMOutputSpec {
	...
	if (!attrs?.href) return ["span", { "data-link": "true" }, 0];
	...
	if (attrs.target) htmlAttrs["data-target"] = attrs.target;
	if (attrs.markdownStyle) {
		htmlAttrs["data-markdown-style"] = attrs.markdownStyle;
	}
